...
if is_available rg; then
    ...
    # The "traditional" rg command explicitly checks to make sure it is being
    # invoked as "rg", and otherwise does nothing. I would like to use the rg
    # completion script with my own functions (rgl, rgh, etc.). Copy the
    # "traditional" rg completion script here with one minor change:
    # unilaterally set the `cmd` variable to "rg". This allows the completion
    # script to always run, no matter what my function is called.
    function _rg_hsm()
    {
    ...
    complete -o bashdefault -o default -F _rg_hsm rgl
    complete -o bashdefault -o default -F _rg_hsm rgh
    complete -o bashdefault -o default -F _rg_hsm rghl
fi
